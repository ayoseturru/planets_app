...
const CloseIcon: React.FC<CloseIconProps> = ({onInteraction}) => {
    ...
    const handleInteraction = (event: React.MouseEvent<SVGSVGElement> | React.KeyboardEvent<SVGSVGElement>) => {
        if (event.type === "click" || (event.type === "keydown" && (event as React.KeyboardEvent<SVGSVGElement>).key === "Enter")) {
            onInteraction();
        }
    };
